...
를 계속 진행합니다  volumes,클라우드 볼륨을 마운트합니다.
...
. 템플릿에서 하드 코딩되지 않은 모든 매개변수에 대한 값을 추가합니다. 을 참조하십시오  volumes,볼륨 생성 중 Azure NetApp Files 볼륨을 구축하기 위해 완료해야 하는 모든 매개 변수에 대한 자세한 내용은 를 참조하십시오.
